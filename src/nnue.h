...
void Net::toggleFeature(Position& pos, uint64_t cleanBitboard, int piece, int square) {
    ...
    while (cleanBitboard) {
        ...
        int wOffset = indexWhite * L1_SIZE * 12 + wSq * 32 + L1_SIZE * wPc;
        int bOffset = indexBlack * L1_SIZE * 12 + bSq * 32 + L1_SIZE * bPc;

        for (int i = 0; i < 32; i++) {
            accumulator[WHITE][wSq * 32 + i] += weights0[wOffset + i] * (!STATE ? -1 : 1);
            accumulator[BLACK][bSq * 32 + i] += weights0[bOffset + i] * (!STATE ? -1 : 1);
        }
    }
}
