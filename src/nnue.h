...
void Net::toggleFeature(Position& pos, uint64_t cleanBitboard, int piece, int square) {
    ...
    while (cleanBitboard) {
        ...
        int wOffset = indexWhite * L1_SIZE * 12 + wSq * 16 + L1_SIZE * wPc;
...
inline void Net::refreshMiniAcc(Position& pos, Piece piece, int square) {
    ...
    memcpy(&accumulator[WHITE][ square * 16], &bias0[square  * 16 + L1_SIZE * piece ], 16 * sizeof(int16_t));
    memcpy(&accumulator[BLACK][bSquare * 16], &bias0[bSquare * 16 + L1_SIZE * bPiece], 16 * sizeof(int16_t));
    ...
    while (occupied) {
        ...
        int wOffset = indexWhite * L1_SIZE * 12 +  square * 16 + L1_SIZE *  piece;
