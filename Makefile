...
DEFAULT_POLICY_NAME=quant_dropTheLR_74.bin
...
DEFAULT_VALUE = $(MOLY_DIR)/Nets/$(DEFAULT_VALUE_NAME)

EVALFILE ?= $(DEFAULT_NET)
POLICYFILE ?= $(DEFAULT_POLICY)
VALUEFILE ?= $(DEFAULT_VALUE)
...
CXXFLAGS += -DEVALFILE=\"$(EVALFILE)\" -DPOLICYFILE=\"$(POLICYFILE)\" -DVALUEFILE=\"$(VALUEFILE)\"
